...
{%block fatosdasentença %}

# 1 – Fatos

No processo originário n. **(processo originário)** [íntegra dos autos anexos - doc. 5],** [nome da parte] ingressou com ação judicial em face do Estado de Rondônia, para pleitear o fornecimento do medicamento [nome do medicamento], a fim de realizar tratamento **oncológico**.

A tutela antecipada foi deferida, para obrigar os demandados a fornecer o fármaco, nos seguintes termos:

> (“trecho da decisão em citação direta”)

Em razão do descumprimento, houve **bloqueio de valores, unicamente, em face do Ente Estadual**, **no valor total de R$ [valor total] [valor total por extenso]**, para imediato atendimento da demanda judicial.

Em sentença, julgou-se o pedido [resultado do julgamento: parcialmente procedente, procedente, etc]. 
...
No mais, através do Ofício [número do ofício] (anexo - doc. 1), foi realizada uma solicitação administrativa ao Ministério da Saúde referente ao reembolso dos valores despendidos com a medicação.
...
Assim, diante da dispensação dos medicamentos pelo Estado de Rondônia e da inércia da União Federal em ressarcir os cofres públicos, não houve outra alternativa diferente da propositura da presente demanda judicial.

{% block fundamentacaosei %}
...
É fundamental destacar que tal solicitação administrativa foi devidamente recebida e registrada **sob o n. [número do SEI],** no Sistema Eletrônico de Informações, do Ministério da Saúde (SEI/MS) (doc. 1).
...
### 2.2 – Desnecessidade da União Federal figurar nos autos originários, para ingresso com ação regressiva
...
Sendo assim, o fato de a União não ter participado da lide originária não afasta sua responsabilização em regresso dos valores.
...
1. A solidariedade passiva entre os entes federados em matéria de saúde foi reafirmada.
...
[...]
13\. Quando o magistrado determinar que a **obrigação de fornecer medicamento** fora da lista do SUS seja **cumprida por determinado ente público**, nada impede que, posteriormente, reconheça-se a **possibilidade de ressarcimento por outro, caso se entenda ser deste último o dever de custeio**. Precedente do STJ.” (Destacou-se)
...
**[...]**
**XI - conjugação dos recursos financeiros, tecnológicos, materiais e humanos da União, dos Estados, do Distrito Federal e dos Municípios na prestação de serviços de assistência à saúde da população;”** (Destacou-se)
...
{% block medicamentooncologico %}
### 2.4.1 – Responsabilidade da União pelo financiamento do tratamento oncológico

O objeto do processo originário era o fornecimento do fármaco  (nome do medicamento), ou seja, medicação (nome do tratamento), notadamente de alto custo.
...
[...]
**II - garantir o financiamento para o tratamento do câncer, nos moldes das pactuações vigentes, de acordo com as suas responsabilidades; (Origem: PRT MS/GM 876/2013, Art. 8º, II)**” (Destacou-se)

Fica claro, então, que o financiamento do tratamento oncológico fica exclusivamente a cargo da União.
...
Pelo exposto, tanto o E. STF quanto diversos TRFs entendem pela viabilidade do ressarcimento pela União do valor despendido pelo Estado para a aquisição de medicamentos oncológicos.

{% block valordivida %}
### 2.6 – Valor atualizado da dívida

No processo n. (numero do processo originario), em razão do descumprimento da ordem judicial, houve **bloqueio de valores, unicamente, em face do Ente Estadual**, **no valor total de R$ (valor bloqueado) (valor bloqueado por extenso), sendo utilizado somente o montante de R$ (valor utilizado pela paciente) (valor utilizado pela paciente por extenso) pela paciente**, conforme documentação anexa (docs. 3 e 4).

Portanto, o valor atualizado que a União Federal deverá reaver ao Estado é no importe total de **R$ (valor a ser ressarcido já corrigido) (valor a ser ressarcido já corrigido por extenso)**, de acordo com a planilha de cálculos anexa (doc. 9).
{% endblock %}
...
III – A condenação da União Federal ao reembolso ao Estado de Rondônia da quantia correspondente ao fornecimento de medicação oncológico, em razão de determinação judicial oriunda dos autos n. (número do processo originário);
...
{% block valordacausa %}
Dá-se à causa, para efeitos legais, o valor de **R$ (valor da causa) (valor da causa por extenso).**
{% endblock %}
...
{% block documentosanexos %}
Documentos anexos:
1 - Pedido administrativo;
2 - Ordem Judicial;
3 - Bloqueio Judicial;
4 - Nota fiscal;
5 - Cópia dos autos n. (número do processo originário);
6 -  Acórdão análogos;
